...
// 适用条件，subtype与supertype是在同一条继承链上
...
// !type_leq(type2,nd->get_name())  ===> break cond: type2 <= nd->get_name()
// type_leq(nd->get_name(),type2)   ===> break cond: type2 <  nd->get_name()
// 
  ...
  for(nd = lookup_class(type1); 
      ...
      //type_leq(nd->get_name(),type2); 
      nd = nd->get_parentnd())
      if(semant_debug) cerr << "type_lub: nd: " << nd->get_name() <<  " |  type2: " << type2 << endl;
    if(semant_debug) cerr << "type_lub return nd : " << nd->get_name() << endl;
    return nd->get_name(); 
}
