...
public:
   ...
   virtual void install_formal(Environment * env);
#ifdef Formal_EXTRAS
   Formal_EXTRAS
#endif
};
...
public:
   ...
   void tc(Environment* env) // TODO
   {
      env->var_enterscope();
      for(int i = formals->first(); formals->more(i); i = formals->next(i))
         formals->nth(i)->install_formal(env);
      if(env->var_probe(return_type))
      {
      ...
      env->semant_error(this) << "Inferred return type " << return_type << " does not conform to declared return type " << endl;
      env->var_exitscope();
      return;
   }
   void add_to_table(Environment* env) // TODO
   {
...
public:
   ...
   void tc(Environment* env) // TODO
   {
      if(!env->var_probe(name))
         env->semant_error(this) << "Class " << type_decl << " of attribute " << "xxxx" << " is undefined." << endl;
      if(env->type_leq())
          env->semant_error(this) << "Inferred type " << "xxxx" << " of initialization of attribute " << "xxxx" << " does not conform to declared type " << endl;
   }
   void add_to_table(Environment* env) // TODO
   {
      if (name == self)
...
public:
   ...
   void install_formal(Environment * env) // TODO
   {
      if(type_decl != SELF_TYPE)
      {
         if(!env->var_probe(type_decl))
            env->semant_error(this) << "Class " << type_decl << " of formal parameter " << name << " is undefined." << endl;
      }
      else
         env->semant_error(this) << "Formal parameter " << name << " cannot have type SELF_TYPE." << endl;
      if(name == self)
         env->semant_error(this) << "\'self\' cannot be the name of a formal parameter." << endl;
      if(env->var_probe(name))
...
public:
   ...
   Symbol tc(Environment* env) // TODO
   {/* expr->tc(); ??*/}
...
public:
   ...
   {
      // TODO (*(void (_DWORD, Environment *))  (**(this + 3) + 20))(*(this + 3), a2);
      type = Bool;
      return Bool;
   }
