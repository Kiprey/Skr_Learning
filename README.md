...
- 启航，JS-fuzzer。在写胶水把两个 fuzzer 粘起来时捡到了一个栈溢出洞，￥3k到手
...
- 继续写 Fuzzer 代码，本周用 node 搭配 node C++ 插件把 chrome GUI fuzzer 写完了。
  - 点亮技能树：**node C++ plugin** hello world!
  - 自己独立写一个新 fuzzer 而非沿用之前的，是为了提高 fuzz 速度。
